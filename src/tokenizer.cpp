...
static void end_float_token(Tokenize *t) {
    BigInt significand_bi;
    bigint_init_bigint(&significand_bi, &t->cur_tok->data.int_lit.bigint);
    ...
    if (!bigint_fits_in_bits(&significand_bi, 64, false)) {
        t->cur_tok->data.float_lit.overflow = true;
        return;
    }

    uint64_t significand = bigint_as_unsigned(&significand_bi);
